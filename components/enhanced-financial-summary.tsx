...
export function EnhancedFinancialSummary({
      ...
      <Card>
        <CardContent className="p-6">
          <div className="flex items-center justify-center">
            ...
            <span className="ml-2 text-gray-900">Calculando gastos unificados...</span>
          </div>
        </CardContent>
      </Card>
    )
  ...
  // CORRECCIÓN: Calcular el beneficio neto correctamente
  // Fórmula: Ingresos - Comisión Total - Gastos Operacionales - Gastos Fijos - Gastos Variables
  const beneficioNetoReal = totalIncome - driverCommission - unifiedExpenses.totalExpenses

  const profitMargin = totalIncome > 0 ? (beneficioNetoReal / totalIncome) * 100 : 0
      ...
      <Card>
        ...
        <CardContent>
            ...
            <div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between">
                  <span className="text-gray-800">Total Facturado:</span>
            ...
            <div>
              ...
              <div className="space-y-2">
                ...
                <div className="border-t pt-2 mt-2">
                  <div className="text-xs font-medium text-gray-800 mb-1">GASTOS FIJOS MENSUALES:</div>
                  <div className="flex justify-between text-xs">
                    <span className="text-gray-700">• Seguridad Social:</span>
                    <span className="text-gray-800">
                      {formatCurrency(unifiedExpenses.monthlyFixedExpenses.seguridadSocial)}
                    </span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-gray-700">• Cuota Autónomo:</span>
                    <span className="text-gray-800">
                      {formatCurrency(unifiedExpenses.monthlyFixedExpenses.cuotaAutonomo)}
                    </span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-gray-700">• Cuota Agrupación:</span>
                    <span className="text-gray-800">
                      {formatCurrency(unifiedExpenses.monthlyFixedExpenses.cuotaAgrupacion)}
                    </span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-gray-700">• Gestoría:</span>
                    <span className="text-gray-800">
                      {formatCurrency(unifiedExpenses.monthlyFixedExpenses.gestoria)}
                    </span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-gray-700">• Seguros (trimestral):</span>
                    <span className="text-gray-800">
                      {formatCurrency(unifiedExpenses.monthlyFixedExpenses.seguros)}
                    </span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-gray-700">• Suministros:</span>
                    <span className="text-gray-800">
                      {formatCurrency(unifiedExpenses.monthlyFixedExpenses.suministros)}
                    </span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-gray-700">• Otros Fijos:</span>
                    <span className="text-gray-800">{formatCurrency(unifiedExpenses.monthlyFixedExpenses.otros)}</span>
                  </div>
                </div>

                <div className="border-t pt-2 mt-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-700">⛽ Gastos Operacionales Diarios:</span>
                    <span className="text-blue-600">{formatCurrency(unifiedExpenses.dailyOperationalExpenses)}</span>
                  </div>
                  <div className="text-xs text-gray-600 ml-4">(Combustible + otros gastos del período)</div>
                </div>
          ...
          <div className="mt-6 p-4 bg-gradient-to-r from-blue-50 to-blue-100 rounded-lg border border-blue-200">
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-sm text-blue-800">Después de TODOS los gastos y comisiones</p>
                <p className="text-xs text-blue-700 mt-1">
                  Fórmula: Ingresos - Comisión Total - Gastos Fijos - Gastos Operacionales - Gastos Variables
                </p>
              </div>
              <div className="text-right">
                <div className={`text-2xl font-bold ${beneficioNetoReal > 0 ? "text-green-600" : "text-red-600"}`}>
                  {formatCurrency(beneficioNetoReal)}
                </div>
          ...
          <div className="mt-4 p-3 bg-gray-50 rounded-lg text-sm">
            <h4 className="font-medium mb-2 text-gray-900">🧮 Desglose del Cálculo:</h4>
            <div className="space-y-1 text-xs">
              <div className="flex justify-between">
                <span className="text-gray-800">Ingresos totales:</span>
              ...
              <div className="flex justify-between text-xs text-gray-700 ml-4">
              ...
              <div className="flex justify-between">
                <span className="text-gray-800">Gastos fijos mensuales:</span>
              ...
              <div className="flex justify-between">
                <span className="text-gray-800">Gastos operacionales:</span>
                <span className="text-red-600">-{formatCurrency(unifiedExpenses.dailyOperationalExpenses)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-800">Gastos variables:</span>
                <span className="text-red-600">-{formatCurrency(unifiedExpenses.variableExpenses)}</span>
              </div>
              <div className="border-t pt-1 flex justify-between font-medium">
                <span className="text-gray-900">Beneficio neto:</span>
                <span className={beneficioNetoReal > 0 ? "text-green-600" : "text-red-600"}>
                  {formatCurrency(beneficioNetoReal)}
                </span>
              </div>
            </div>
          ...
          <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
            ...
            <div className="text-sm space-y-1">
              <div className="flex justify-between">
                <span className="text-gray-800">Comisión acumulada (35%):</span>
                <span className="font-medium text-gray-900">{formatCurrency(driverCommission)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-800">Nómina mensual:</span>
                <span className="font-medium text-gray-900">{formatCurrency(nominaReal)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-800">Efectivo a recibir:</span>
                <span className="font-medium text-green-600">{formatCurrency(efectivoAdicional)}</span>
              </div>
              {efectivoAdicional > 0 && (
                <div className="text-xs text-green-800 mt-2 p-2 bg-green-100 rounded">
                  ✅ El conductor ha superado su nómina base y recibirá {formatCurrency(efectivoAdicional)} adicionales
                  en efectivo.
                </div>
              )}
              {driverCommission < NOMINA_BASE && (
                <div className="text-xs text-orange-800 mt-2 p-2 bg-orange-100 rounded">
          ...
          {/* Alertas */}
          {!isHealthyMargin && beneficioNetoReal > 0 && (
